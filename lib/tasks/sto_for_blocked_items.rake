require 'csv'
namespace :sto_for_blocked_items do
  desc 'Create STO and completed with QC'
  task :create, %i(starting_rto_id ending_rto_id) => :environment do |t, args|
  ...
  def dh_assign_warehouse_variant_location(dh_po_id, warehouse = dhaka_warehouse)
    ...
    dh_po.line_items.each do |dh_po_li|
      ...
      # wv.update!(available_quantity: wv.available_quantity + 0) # 0 increment because we added this qty without any sto while business wanted
      # wvl.update!(quantity: (wvl.quantity + dh_po_li.qc_passed)) # should not add into location because when unblock them they will assign into location
