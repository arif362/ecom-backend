require 'csv'
namespace :return_transfer_orders do
  desc 'Create IGT and completed with QC'
  task :create_igt, [:warehouse_id, :staff_id] => :environment do |t, args|
    rto_rows = []
    ...
    ActiveRecord::Base.transaction do
      WarehouseVariant
        ...
        rto_rows = rto_rows + return_transfer_order_full_cycle(warehouse, variant_id_with_quantities, current_staff)
      end
    end
  ...
  def return_transfer_order_full_cycle(warehouse, variant_id_with_quantities, current_staff)
    ...
    ActiveRecord::Base.transaction do
      ...
      return_orders.each do|return_order|
        ...
        rto_rows = rto_rows + assign_warehouse_variant_location(return_order.id, central_warehouse)
      end
    end
  ...
  def assign_warehouse_variant_location(rto_id, warehouse)
    ...
    return_transfer_order.line_items.each do |rto_li|
      ...
      wv.update!(available_quantity: wv.available_quantity + rto_li.qc_passed)
      wvl.update!(quantity: (wvl.quantity + rto_li.qc_passed))
  ...
  def create_csv_file_to_track_as_rto_id_wvl_id_quantity(warehouse, rto_rows, f_mode = 'w')
    ...
    rto_rows = [['return_transfer_order_id', 'location_id', 'quantity']] + rto_rows if f.size.zero?
