    ...
    module Entities
      class BnCartDetails < Grape::Entity
        ...
        def product_title(variant)
          product = Product.unscoped.find_by(id: variant.product_id, is_deleted: false)
          "#{product&.bn_title&.to_s} #{variant.product_attribute_values&.map(&:bn_value)&.join('-')&.to_s} (#{variant.sku&.to_s})"
        end
