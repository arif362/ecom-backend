module ShopothPartner
  module V1
    module Entities
      class PartnerProducts < Grape::Entity
        include ShopothPartner::V1::Helpers::ImageHelper

        expose :id, as: :product_id
        expose :title do |model, options|
          options[:language] == 'en' ? model.title : model.bn_title
        end
        expose :description do |model, options|
          options[:language] == 'en' ? model.description : model.bn_description
        end
        expose :short_description do |model, options|
          options[:language] == 'en' ? model.short_description : model.bn_short_description
        end
        expose :warranty_period
        expose :warranty_policy do |model, options|
          options[:language] == 'en' ? model.warranty_policy : model.bn_warranty_policy
        end
        expose :video_url
        expose :warranty_type # enum
        expose :warranty_period_type
        expose :company do |model, options|
          options[:language] == 'en' ? model.company : model.bn_company
        end
        expose :en_brand, as: :brand
        expose :certification do |model, options|
          options[:language] == 'en' ? model.certification : model.bn_certification
        end
        expose :license_required
        expose :material do |model, options|
          options[:language] == 'en' ? model.material : model.bn_material
        end
        expose :consumption_guidelines do |model, options|
          options[:language] == 'en' ? model.consumption_guidelines : model.bn_consumption_guidelines
        end
        expose :temperature_requirement do |model, options|
          options[:language] == 'en' ? model.temperature_requirement : model.bn_temperature_requirement
        end
        expose :keywords
          ...
          product_attribute.map do |key, value|
            {
                ...
                product_attribute_values: value.map { |h| { id: h[:id], value: h[:value], images: h[:images] } },
            }
          end
        end
